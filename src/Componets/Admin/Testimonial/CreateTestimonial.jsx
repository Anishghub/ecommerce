...
import {  addTestimonial } from "../../../Store/ActionCreater/TestimonialActionCreaters"
      ...
      <div className="container-fluid">
        <div className="row">
          ...
          <div className="col-md-9">
            <h4 className='bg-primary text-center text-light p-2'>Testimonial</h4>
            <form onSubmit={postData}>
              <div className="row">
                <div className="col-md-6 mb-3">
                  <label>Name <span className='text-danger'>***</span></label>
                  <input type="text" name="name" onChange={getInputData} className='form-control' placeholder='Person name' />
                  {show ? <p className='text-danger texr-capitalize my-2'>{errorMessages.name}</p> : ""}
                </div>
                <div className="col-md-6 mb-3">
                  ...
                  {show ? <p className='text-danger texr-capitalize my-2'>{errorMessages.profession}</p> : ""}
                </div>
              </div>
              <div className="row">
                <div className="col-md-6 mb-3">
                  ...
                  {show ? <p className='text-danger texr-capitalize my-2'>{errorMessages.star}</p> : ""}
                </div>
                <div className="col-md-6 mb-3">
                  <label>Pic</label>
                  <input type="file" name="pic" onChange={getInputFile} className='form-control' />
                  {show ? <p className='text-danger texr-capitalize my-2'>{errorMessages.pic}</p> : ""}
                </div>
              </div>
              <div className="mb-3">
                <label> Message <span className='text-danger'>*</span></label>
                <textarea name="message" rows="5" onChange={getInputData} placeholder='Message.... ' className='form-control'></textarea>
                {show ? <p className='text-danger texr-capitalize my-2'>{errorMessages.message}</p> : ""}
